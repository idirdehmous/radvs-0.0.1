...
def Radar_info(fid):
    """Searching informations about 
     Radar station coordinates and height    (  "where"   group)
    ...
    for item in groupid.keys():
        ...
        if 'what'  in item:
          ...
          try:
             ...
             wmoid='None' 
             node='None'
    return lat , lon , height,wmoid ,node 
...
# Additional attributes in how group if  it existes
    rpm     =[]
    plswidth=[]
    vsamples=[]
    ...
    if wr_att in groupid.keys():
       if len(list(groupid[wr_att].attrs.keys())) != 0:
          ...
          #print 'Dataset :', dataset , ' has empty ',wr_att,' attribute' 
          pass 
...
if __name__=='__main__':
   ...
   if args.file == None:
      ...
      lat , lon , height,wmoid ,node=Radar_info(fid)
      print '-'*45
      print 'RADAR STATION GLOBAL INFORMATIONS '
      print '-'*45
      print 'Latitude             :', lat 
      print 'Longitude            :', lon
      print 'Height               :', height
      print 'WMO ID               :', wmoid 
      print 'Node or station name :', node 
